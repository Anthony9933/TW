...
const { createRedisClient, isRedisConfigured } = require("./redis-client");

const PREFIX = "sw";
const MAX_EVENTS = 100;

// gpt-4o-mini pricing (USD per 1M tokens) for accurate cost estimates.
...
async function redisSadd(setKey, member) {
  ...
  try {
    ...
    return await redis.sadd(setKey, member);
  } catch (e) {
...
async function recordOutbound(from, usage = null) {
  ...
  const tokens = usage?.total_tokens || inTok + outTok || 0;

  memory.totals.outbound += 1;
  bumpMemoryDaily("outbound", tokens ? { openai_tokens: tokens } : {});
  if (tokens) memory.totals.openai_tokens += tokens;
  if (inTok) memory.totals.openai_input_tokens += inTok;
  if (outTok) memory.totals.openai_output_tokens += outTok;
  ...
  if (inTok) await redisIncrby(`${PREFIX}:totals:openai_input_tokens`, inTok);
  if (outTok) await redisIncrby(`${PREFIX}:totals:openai_output_tokens`, outTok);
...
async function getTotalsFromRedis() {
  ...
  try {
    const keys = [
      ...
      `${PREFIX}:totals:openai_input_tokens`,
      `${PREFIX}:totals:openai_output_tokens`,
    ];
    const vals = await redis.mget(...keys);
    const [inbound, outbound, errors, forget, tok, inTok, outTok] = vals || [];
    return {
      ...
      openai_input_tokens: Number(inTok) || 0,
      openai_output_tokens: Number(outTok) || 0,
    };
  } catch (e) {
...
/**
 * USD estimate for gpt-4o-mini. Uses the real input/output split when we have
 * it; falls back to a blended rate for legacy totals recorded before the split.
 */
function estimateCostUsd(totals = {}) {
  const inTok = totals.openai_input_tokens || 0;
  const outTok = totals.openai_output_tokens || 0;
  if (inTok || outTok) {
    return (inTok * OPENAI_INPUT_PER_M + outTok * OPENAI_OUTPUT_PER_M) / 1_000_000;
  }
  // Legacy fallback: only a combined total exists, so blend the two rates.
  const blended = (OPENAI_INPUT_PER_M + OPENAI_OUTPUT_PER_M) / 2;
  return ((totals.openai_tokens || 0) / 1_000_000) * blended;
}
...
async function recordStateUpdate(from, state = {}) {
  ...
  // Track conversation language (en/ar). Counted once per user, the first time
  // we observe their language, so this reflects distinct users not message volume.
  if (state.language) {
    const added = await redisSadd(
      `${PREFIX}:language:${state.language}:users`,
      h
    );
    // redisSadd returns undefined when Redis isn't configured; only increment the
    // counter when this hash was newly added to the set (added === 1).
    if (added === 1) {
      await redisIncr(`${PREFIX}:language:${state.language}`);
    }
  }
...
/**
 * Get conversation language breakdown (distinct users per language).
 */
async function getLanguageBreakdown() {
  ...
  try {
    const keys = languages.map((l) => `${PREFIX}:language:${l}`);
    const vals = await redis.mget(...keys);

    const result = {};
    languages.forEach((lang, i) => {
      result[lang] = Number(vals[i]) || 0;
    });

    return result;
  } catch (e) {
    console.error("[analytics] getLanguageBreakdown", e);
    return null;
  }
}
 ...
 */
async function recordGoalCreated(from, goalCategory) {
  const h = hashUser(from);

  // Increment total goals created
  await redisIncr(`${PREFIX}:goals:created`);

  // Track by category
  await redisIncr(`${PREFIX}:goals:category:${goalCategory}:created`);

  // Track in events
  pushEvent("goal_created", h, { category: goalCategory });
  await redisLpushEvent({
    ts: Date.now(),
    type: "goal_created",
    user: h,
 ...
 */
async function recordGoalCompleted(from, goalCategory) {
  const h = hashUser(from);

  // Increment total goals completed
  await redisIncr(`${PREFIX}:goals:completed`);

  // Track by category
  await redisIncr(`${PREFIX}:goals:category:${goalCategory}:completed`);

  // Track in events
  pushEvent("goal_completed", h, { category: goalCategory });
  await redisLpushEvent({
    ts: Date.now(),
    type: "goal_completed",
    user: h,
...
/**
 * Get goal analytics (total created, completed, completion rate).
 */
async function getGoalAnalytics() {
  if (!redis) return null;

  try {
    const [created, completed] = await redis.mget(
      `${PREFIX}:goals:created`,
      `${PREFIX}:goals:completed`
    );

    const totalCreated = Number(created) || 0;
    const totalCompleted = Number(completed) || 0;
    ...
    for (const cat of categories) {
      const [catCreated, catCompleted] = await redis.mget(
        `${PREFIX}:goals:category:${cat}:created`,
        `${PREFIX}:goals:category:${cat}:completed`
      );
      const created = Number(catCreated) || 0;
      const completed = Number(catCompleted) || 0;

      if (created > 0) {
        categoryStats[cat] = {
          created,
          completed,
          completion_rate: Math.round((completed / created) * 100),
        };
      }
    }
    ...
    console.error("[analytics] getGoalAnalytics", e);
    return null;
  }
}
 ...
 */
/**
 * Probe the store rather than assuming a constructed client means a working
 * one. Reporting "redis" while every command fails is what let a deleted
 * database go unnoticed: the dashboard looked healthy and simply showed zeros.
 *
 * @returns {Promise<"redis"|"redis-unreachable"|"redis-misconfigured"|"memory">}
 */
async function probeStorage() {
  if (!isRedisConfigured()) return "memory";
  if (!redis) return "redis-misconfigured";
  try {
    await redis.ping();
    return "redis";
  } catch (e) {
    console.error("[analytics] Redis ping failed:", e.message);
...
async function getDashboardSnapshot() {
  ...
  const goals = await getGoalAnalytics();
  ...
  return {
    ...
    totals: {
      ...totals,
      estimated_openai_cost_usd: estimateCostUsd(totals),
    },
    today,
    series_14d: series,
    unique_users_7d,
    ...
    storage,
    storage_healthy: storage === "redis" || storage === "memory",
