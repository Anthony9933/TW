...
/**
 * Load a conversation, reporting whether the backing store was actually
 * readable.
 *
 * The distinction matters: a `null` session from a healthy store means "new
 * user", while a `null` session from a broken store means "unknown". Collapsing
 * the two is what made a dead Redis instance replay the first-contact flow on
 * every single message instead of degrading gracefully.
 *
 * @returns {Promise<{session: Object|null, ok: boolean, durable: boolean}>}
 *   `ok`      — the read completed without error
 *   `durable` — the answer came from storage that survives between serverless
 *               invocations. The in-memory map does not, so a cold start there
 *               looks identical to a brand-new user.
 */
async function getSessionResult(phoneNumber) {
  ...
  if (redis) {
    try {
      ...
      const session = raw
        ? { ...createEmptySession(), ...(typeof raw === "string" ? JSON.parse(raw) : raw) }
        : null;
      // Mirror into memory so a warm instance survives a later Redis blip.
      if (session) memoryStore.set(key, session);
      return { session, ok: true, durable: true };
    } catch (err) {
      console.error("[luna] Redis get failed, falling back to memory:", err.message);
      return { session: memoryStore.get(key) || null, ok: false, durable: false };
    }
  }

  return { session: memoryStore.get(key) || null, ok: true, durable: false };
}
...
async function getSession(phoneNumber) {
  const { session } = await getSessionResult(phoneNumber);
  return session;
}
...
async function saveSession(phoneNumber, session) {
  ...
  // Always keep the in-memory copy current so a warm instance stays coherent
  ...
  if (redis) {
    try {
      ...
      console.error("[luna] Redis set failed, kept in memory only:", err.message);
  ...
  if (!hasWarnedNoRedis && process.env.NODE_ENV !== "test") {
    ...
    console.warn(
      "[luna] Redis not configured. Using in-memory store (data is lost on restart). " +
        "Set UPSTASH_REDIS_REST_URL + UPSTASH_REDIS_REST_TOKEN, or REDIS_URL, for persistence."
    );
  }
  return { persisted: false };
}
...
async function deleteSession(phoneNumber) {
  ...
  if (redis) {
    try {
      ...
      console.error("[luna] Redis del failed:", err.message);
    }
  }
}
...
/**
 * True when Redis credentials are present but the client could not be built or
 * a probe fails. Surfaced on the dashboard so a dead database is visible rather
 * than silently reported as healthy storage.
 */
async function checkStorageHealth() {
  if (!isRedisConfigured()) return { storage: "memory", healthy: true };
  if (!redis) return { storage: "redis-misconfigured", healthy: false };
  try {
    await redis.ping();
    return { storage: "redis", healthy: true };
  } catch (err) {
    return { storage: "redis-unreachable", healthy: false, error: err.message };
  }
}
